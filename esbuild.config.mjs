...
const openDyslexicLicense = fs.readFileSync(path.join("fonts", "OpenDyslexic-OFL.txt"), "utf8");
...
async function loadOpenDyslexicWoff2() {
  const local = path.join("fonts", "OpenDyslexic-Regular.woff2");
  try {
    const buf = fs.readFileSync(local);
    if (buf.length > 1000 && buf.subarray(0, 4).toString() === "wOF2") return buf;
  } catch {}
  ...
  console.log(`OpenDyslexic: downloaded ${buf.length} bytes to ${local}`);
  return buf;
}

const injectOpenDyslexicFont = {
  name: "inject-opendyslexic-font",
  setup(build) {
    build.onLoad({ filter: /[/\\]src[/\\]main\.js$/ }, async (args) => {
      let code = await fs.promises.readFile(args.path, "utf8");
      if (!code.includes('id: "opendyslexic"')) {
        const needle = `  inter: {
    id: "inter",
  ...
  systemSans:`;
        ...
        if (!code.includes(needle)) throw new Error("inject-opendyslexic-font: insertion point missing");
...
const ctx = await esbuild.context({
  ...
  metafile: true,
  banner: { js: banner },
  plugins: [...profile.plugins, neutralise, foliate.plugin, injectOpenDyslexicFont],
  charset: "utf8",
