...
function createPdfLazyView(doc, loadingTask, pageText) {
  return {
        ...
        for (const [scale, budget] of [[fit, 15000], [fit / 2, 8000]]) {
          ...
          try {
            ...
            const blob = typeof canvas.toBlob === "function"
              ...
              : null;
            const src = blob ? URL.createObjectURL(blob) : canvas.toDataURL("image/jpeg", 0.82);
            if (blob) this._objectUrls.add(src);
            return { src, textLayer };
          } catch (e) {
    ...
    release(src) {
      if (!this._objectUrls.delete(src)) return;
      try { URL.revokeObjectURL(src); } catch { /* document is already closing */ }
    },
    async cleanup() {
      if (this._destroyed) return;
      // Text-layer hit boxes still depend on the PDF fonts after rendering.
      // Release page/operator caches while retaining only those shared fonts.
      try { await doc.cleanup(true); } catch { /* the next render can rebuild resources */ }
    },
    destroy() {
      if (this._destroyed) return;
      this._destroyed = true;
      for (const src of this._objectUrls) {
        try { URL.revokeObjectURL(src); } catch { /* document is already closing */ }
      }
...
async function extractPdf(file, app, _settings = {}, onProgress, options = {}) {
  ...
  try {
    ...
    try { await doc.cleanup(); } catch { /* lazy rendering can still continue */ }
    return {
      html: parts.join("\n"),
...
async function drawFigure(img, lazy, current = () => true) {
  ...
  try {
    ...
    if (!current()) { lazy.release(rendered.src); return; }
    const previous = img.getAttribute("src");
    img.src = rendered.src;
    if (previous) lazy.release(previous);
...
async function sweepReaderFigures(reader, lazy) {
  ...
  for (const img of [...flow.querySelectorAll(FIGURE_LAZY_SELECTOR)]) {
    ...
    if (gap <= FIGURE_LOAD_SPAN && loadState !== "1" && loadState !== "skip") {
      ...
      if (src) { img.removeAttribute("src"); lazy.release(src); }
      const surface = img.closest(FIGURE_SURFACE_SELECTOR);
